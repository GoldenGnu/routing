...
public class GreedyCrossoverHibrid2OptEngine extends GreedyCrossoverEngine {
    ...
    * @param cities - chromosome to be optimalized
 	*/
   public static void heuristics2opt(Node[] cities) {
       boolean done = false;
       ...
       for(int k = 0; k < count && !done; k++)
       {
           ...
           for(int i = 0; i < count; i++)
           {
               for(int j = i + 2; j < count; j++)
                   if(
                      Manager.distance(cities[i], cities[(i + 1) % count]) + Manager.distance(cities[j], cities[(j + 1) % count]) 
                		   > 
                   	   Manager.distance(cities[i] , cities[j]) +  Manager.distance(cities[(i + 1) % count], cities[(j + 1) % count])
                   )
                   {
                       Node tmp = cities[(i + 1) % count];
