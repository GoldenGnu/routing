...
public abstract class AbstractEngine<T extends Node> extends RoutingAlgorithm<T> {

    long lastTime = -1;
    int lastDistance = -1;
    short[][] distances;

    public AbstractEngine(String name, String technicalDescription) {
        super(name, technicalDescription);
    }

    public List<T> execute(Progress progress, Graph<T> g, List<T> assetLocations, TSPEngine<T> engine) {
        ...
        List<T> nodes = new ArrayList<>(assetLocations);

        Manager.initialize(nodes, distances, 4509809);

        engine.initialize(configuration, nodes.toArray((T[]) Array.newInstance(nodes.iterator().next().getClass(), 0)), loop);

        boolean stopRequestFlag = false;
        int bestCostAge = 0;
        TSPChromosome<T> bestChromosome = null;
        double bestCost;
        ...
        ArrayList<T> best = new ArrayList<>(Arrays.asList(bestChromosome.getCities()));
