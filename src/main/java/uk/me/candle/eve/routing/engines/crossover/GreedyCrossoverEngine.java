...
public class GreedyCrossoverEngine extends SimpleUnisexMutatorHibrid2OptEngine {
	...
	protected void getChild(TSPChromosome parent1, TSPChromosome parent2) {
		...
		Node child1[] = parent1.getCities().clone();
		Node child2[] = parent2.getCities().clone();
		Node child3[] = haveSex(parent1, parent2);
		Node child4[] = haveSex(parent2, parent1);
		Node child5[] = child3.clone();
		Node child6[] = child4.clone();
	 ...
	 */
	protected Node[] haveSex(TSPChromosome chromosome1, TSPChromosome chromosome2) {

		Node[] c1 = chromosome1.getCities();
		Node[] c2 = chromosome2.getCities();

		int n = c1.length;

		LinkedList<Node> out = new LinkedList<Node>();
		LinkedList<Node> not_picked = new LinkedList<Node>();
		...
		while (not_picked.size() > 1) {
			Node last = out.getLast();
			Node n1 = findNext(c1, last);
			Node n2 = findNext(c2, last);
	 ...
	 */
	protected Node findNext(Node[] cities, Node x) {
